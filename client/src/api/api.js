...
export const notificationsApi = {
  ...
  async addNotification(notif) {
    const { data } = await api.post(
      `http://localhost:5000/notification/add`,
      notif
    );
    return data;
  },
  async putNotification(id) {
    const { data } = await api.put(
      `http://localhost:5000/notification/update/${id}`
    );
    return data;
  },
  async deleteNotification(id) {
    const { data } = await api.delete(
      `http://localhost:5000/notification/delete/${id}`
    );
    return data;
  },
};
...
export const RecomandationApi = {
  async getRecomandationCourse(category) {
    const { data } = await api.get(
      `http://localhost:5000/recomandation/recomandedcourse/${category}`
    );
    return data;
  },
};
